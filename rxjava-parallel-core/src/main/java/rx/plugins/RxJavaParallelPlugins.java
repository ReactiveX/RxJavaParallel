 ...
 */
public class RxJavaParallelPlugins {
    private final static RxJavaParallelPlugins INSTANCE = new RxJavaParallelPlugins();

    private final AtomicReference<RxJavaErrorHandler> errorHandler = new AtomicReference<RxJavaErrorHandler>();
    private final AtomicReference<RxJavaParallelObservableExecutionHook> observableExecutionHook = new AtomicReference<RxJavaParallelObservableExecutionHook>();
     ...
     */
    public static RxJavaParallelPlugins getInstance() {
        return INSTANCE;
    }

    /* package accessible for unit tests */RxJavaParallelPlugins() {
     ...
     * <p>
     * Override the default by calling {@link #registerObservableExecutionHook(RxJavaParallelObservableExecutionHook)}
     ...
     */
    public RxJavaParallelObservableExecutionHook getObservableExecutionHook() {
        if (observableExecutionHook.get() == null) {
            // check for an implementation from System.getProperty first
            Object impl = getPluginImplementationViaProperty(RxJavaObservableExecutionHook.class);
            if (impl == null) {
                // nothing set via properties so initialize with default
                observableExecutionHook.compareAndSet(null, RxJavaParallelObservableExecutionHookDefault.getInstance());
                // we don't return from here but call get() again in case of thread-race so the winner will always get returned
            } else {
                // we received an implementation from the system property so use it
                observableExecutionHook.compareAndSet(null, (RxJavaParallelObservableExecutionHook) impl);
            }
        }
     ...
     *             to register)
     */
    public void registerObservableExecutionHook(RxJavaParallelObservableExecutionHook impl) {
